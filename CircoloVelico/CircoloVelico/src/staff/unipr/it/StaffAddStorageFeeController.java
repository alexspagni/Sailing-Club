...
/**
 * The {@code StaffAddStorageFeeController} class is used to show to a Staff
 * member a table where he can choose one fee related to a user and add it to
 * the database.
 * 
 * @author Spagni Alex Zuelli Arianna
 * 
 */
public class StaffAddStorageFeeController implements Initializable {

//	private static final Constant constant = new Constant();
	private FeeStaffEnum getStorageFee=FeeStaffEnum.GET_MEMBERS_MUST_PAY_STORAGE_FEE;
	private FeeStaffEnum addStorageFee=FeeStaffEnum.ADD_MEMBERS_MUST_PAY_STORAGE_FEE;
	...
	@FXML
	private TableColumn<Notification, String> boatNameColumnstaff;
//	@FXML
//	private TableColumn<Notification, Integer> idBoatColumnstaff;
	@FXML
	...
	public void addFee(ActionEvent event) {
		try {
			...
			if (userSelected.isBlank()) {
				...
				int ris = staffService.addUserMustPayStorageFee(addStorageFee.getTypeReq(),
						notificationTemp.getIdmember(), notificationTemp.getDate(), notificationTemp.getPrice(), "no",
						notificationTemp.getIdBoat());
				if (ris >= 0) {
					title = "Send notification form";
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
			...
			ArrayList<Notification> notifications = staffService
					.getUserMustPayFee(getStorageFee.getTypeReq());
			for (Notification i : notifications) {
				values.add(i);
			}
			for (Notification i: values) {
				System.out.println(i.getIdmember()+" "+i.getPrice()+" "+ i.getIdBoat()+" "+i.getBoatName());
			}
			...
			boatNameColumnstaff
					.setCellValueFactory(new PropertyValueFactory<Notification, String>("boatName"));
//			idBoatColumnstaff
//			.setCellValueFactory(new PropertyValueFactory<Notification, Integer>("idBoat"));
					...
					.addListener(new ChangeListener<Notification>() {
						...
						public void changed(ObservableValue<? extends Notification> arg0, Notification arg1,
							...
							notificationTemp = staffSendNotificationPrintTableView.getSelectionModel()
									.getSelectedItem();
							userSelectedTextField.setText(notificationTemp.getBoatName());
						}
