...
/**
 * The {@code StaffSendStorageNotification} class is used to show to a Staff
 * member a table where he can see all storage user fee haven't paied yet. He
 ...
 * 
 */
public class StaffSendStorageNotification implements Initializable {
	private NotificationStaffEnum storageNotification = NotificationStaffEnum.GET_MEMBERS_TO_NOTIFY_STORAGE_FEE;
	private NotificationStaffEnum sendStorageNotification = NotificationStaffEnum.SEND_STORAGE_NOTIFICATION;
//	private static final Constant constant = new Constant();
	...
	public void sendNotification(ActionEvent event) {
		try {
			...
			if (userSelected.isBlank()) {
				...
				int ris = staffService.sendStorageFeeNotification(sendStorageNotification.getTypeReq(),
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
			// Socket client = new Socket(SHOST, SPORT);
			StaffService staffService = new StaffService();
			ArrayList<Notification> notifications = staffService.getUserMustPayFee(storageNotification.getTypeReq());
...
//			}
			...
			boatNameColumnstaff.setCellValueFactory(new PropertyValueFactory<Notification, String>("boatName"));
					...
					.addListener(new ChangeListener<Notification>() {
						...
						public void changed(ObservableValue<? extends Notification> arg0, Notification arg1,
							...
							notificationTemp = staffSendNotificationPrintTableView.getSelectionModel()
									.getSelectedItem();
							userSelectedTextField.setText(Integer.toString(notificationTemp.getIdBoat()));
						}
